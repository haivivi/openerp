...
pub fn expand(attr: TokenStream, item: ItemStruct) -> syn::Result<TokenStream> {
    ...
    // Preserve user doc comments and non-derive attributes.
    let doc_attrs: Vec<_> = item.attrs.iter()
        .filter(|a| a.path().is_ident("doc"))
        .collect();

    // Check which derives the user already has.
    ...
    if !user_derives.contains(&"PartialEq".to_string()) {
        extra_derives.push(quote!(PartialEq));
    }

    // Build derive list: user's existing + our additions.
    ...
    Ok(quote! {
        ...
        impl #struct_name {
            /// The state path in the Flux store.
            pub const PATH: &'static str = #path;
        }
    })
}

/// Parse the path string from attribute: `#[state("auth/state")]`
fn parse_path(attr: TokenStream) -> syn::Result<String> {
    let lit: syn::LitStr = syn::parse2(attr)?;
    let path = lit.value();
    if path.is_empty() {
        return Err(syn::Error::new(lit.span(), "state path cannot be empty"));
    }
    Ok(path)
}

/// Collect derive trait names from existing attributes.
