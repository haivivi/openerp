...
// ── IsDslEnum marker trait ──

/// Auto-trait marker for types generated by `#[dsl_enum]`.
///
/// Used by `#[model]` to distinguish `#[dsl_enum]` types from regular structs
/// when inferring the UI widget. Only types implementing this trait should
/// get the "select" widget automatically.
///
/// This trait is automatically implemented by the `#[dsl_enum]` macro.
pub trait IsDslEnum {
    /// Returns true for `#[dsl_enum]` types.
    const IS_DSL_ENUM: bool = true;
}

/// Helper trait to check if a type is a dsl_enum at compile time.
/// Used by the #[model] macro to determine widget type.
///
/// Only #[dsl_enum] types implement this trait (with IS_DSL_ENUM = true).
/// All other types do not implement it (IS_DSL_ENUM = false is the default).
pub trait MaybeDslEnum {
    const IS_DSL_ENUM: bool;
}
