...
mod tests {
    ...
    // Provide handler implementations via impl_handler!.
    fn run_routes(_kv: std::sync::Arc<dyn openerp_kv::KVStore>) -> axum::Router {
        axum::Router::new()
    }

    fn reset_routes(_kv: std::sync::Arc<dyn openerp_kv::KVStore>) -> axum::Router {
        axum::Router::new()
    }

    openerp_macro::impl_handler!(handler_check::Run, run_routes);
    openerp_macro::impl_handler!(handler_check::Reset, reset_routes);

    #[test]
    fn golden_handler_traits_exist() {
        // Traits generated by #[action] — compile-time verification.
    ...
    fn golden_handler_registry_exists() {
        // __Handlers struct generated by #[facet] with actions.
        let _ = std::mem::size_of::<handler_check::__Handlers>();
    }

    #[test]
    fn golden_action_router_compiles() {
        // action_router::<__Handlers>() compiles because both
        // impl_handler! calls above satisfy the trait bounds.
        fn _check(kv: std::sync::Arc<dyn openerp_kv::KVStore>) {
            let _router = handler_check::action_router::<handler_check::__Handlers>(kv);
        }
    }

    #[test]
    fn golden_no_handler_check_for_action_free_facet() {
        // public_org has no actions — no __Handlers or action_router generated.
        // This test verifies action-free facets still work normally.
        let ir = public_org::__facet_ir();
        assert_eq!(ir["actions"].as_array().unwrap().len(), 0);
    }
    ...
    fn golden_handler_check_coexists_with_client() {
        // Handler check and client are independent features.
        // Both should work on the same facet.
        fn _check_handler(kv: std::sync::Arc<dyn openerp_kv::KVStore>) {
            let _router = handler_check::action_router::<handler_check::__Handlers>(kv);
        }
        fn _check_client() {
